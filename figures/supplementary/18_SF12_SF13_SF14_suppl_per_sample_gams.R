...
pw_df <- read.csv(cfg_path("output_root", "9b_scoring", "pathway_gene_sets_v2.csv"),
...
arch <- readRDS(cfg_path("output_root", "28_glandular_architecture", "per_cell_architecture_wt.rds"))
...
vasc <- read.csv(cfg_path("output_root", "22_vascular_proximity", "vascular_distance_all_cells.csv"),
                 stringsAsFactors = FALSE)
vasc_wt <- vasc[vasc$tissue == "whole_tissue", ]
stopifnot(nrow(vasc_wt) == nrow(arch))
stopifnot(all(vasc_wt$sample_id == arch$sample_id))
arch$dist_to_vascular <- vasc_wt$dist_to_vascular
rm(vasc, vasc_wt)
...
nf <- readRDS(cfg_path("output_root", "16b_niche_succession_gams_v2", "neighborhood_features.rds"))
...
epi <- readRDS(cfg_path("output_root", "19d_gene_polarization_gams",
...
tme <- readRDS(cfg_path("output_root", "19e_gene_gams_all_celltypes",

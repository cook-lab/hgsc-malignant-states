...
# dotplot_stats is a deposited atlas cache (read from data_root, like the other
# atlas figure scripts). lilra4_stats (the LILRA4/pDC marker row) is NOT deposited;
# it is regenerated by atlas/02_annotation/09f_dotplot_canonical_markers.py into
# output_root — the LILRA4 row is skipped gracefully if that step has not been run.
DOTPLOT_CSV = path("data_root", "2026_final_atlas", "output", "09f_dotplot_canonical_markers", "dotplot_stats.csv")
LILRA4_CSV = path("output_root", "02_annotation", "09f_dotplot_canonical_markers", "lilra4_stats.csv")
...
if os.path.exists(LILRA4_CSV):
    df = pd.concat([df, pd.read_csv(LILRA4_CSV)], ignore_index=True)
else:
    print(f"  [warn] {LILRA4_CSV} not found — LILRA4/pDC row omitted. "
          "Run atlas/02_annotation/09f_dotplot_canonical_markers.py to regenerate it.",
          flush=True)

# ============================================================================
# 03_pairwise_tma_morphometrics.R
# ----------------------------------------------------------------------------
# PURPOSE: Pairwise TMA morphometric comparisons across epitypes (per-patient, subsampled).
#
# INPUTS:
#   - output/33_morphometrics/ per-cell TMA morphometrics
#
# OUTPUTS:
#   - output/33_morphometrics/per_patient_summary_tma.csv + pairwise stats
#
# MANUSCRIPT PANEL(S): Fig 5D/5E (TMA)
# RUNTIME TIER: moderate
#
# Migrated from 2026_final_xenium_analysis/scripts/. Analytical logic preserved;
# paths routed through central config, seed from CFG$seed, epithelial label
# "Transitioning" -> "Intermediate", SecA/SecB from shared/signatures.yml.
# ============================================================================

# --- Config + shared setup (replaces hardcoded /Volumes/CookLab/Sarah paths) ---
here <- tryCatch(dirname(sys.frame(1)$ofile), error = function(e) ".")
source(file.path(here, "..", "..", "config", "config.R"))   # CFG, cfg_obj, cfg_path
source(file.path(here, "..", "00_setup", "00_setup.R"))      # load_sfe, save_sfe, theme_lab, nb_names, palettes
set.seed(CFG$seed)

suppressPackageStartupMessages({
  library(data.table)
})

OUT_DIR <- file.path(out_dir, "33_morphometrics")

cache <- readRDS(file.path(OUT_DIR, "per_cell_morphometrics.rds"))
tma <- cache$tma

FEATURES <- c("cell_area","nuc_area","nc_ratio",
                "cell_perimeter","nuc_perimeter",
                "cell_circularity","nuc_circularity",
                "cell_solidity",
                "cell_eccentricity","nuc_eccentricity",
                "nc_centroid_offset")

PREDICT <- c(
  cell_area          = "down",
  nuc_area           = "up",
  nc_ratio           = "up",
  cell_perimeter     = "up",
  nuc_perimeter      = "up",
  cell_circularity   = "down",
  nuc_circularity    = "down",
  cell_solidity      = "down",
  cell_eccentricity  = "up",
  nuc_eccentricity   = "up",
  nc_centroid_offset = "down"
)

PAIRWISE <- list(
  list(label = "SecA_to_Trans", a = "SecA epithelium",
       b = "Intermediate epithelium"),
  list(label = "Trans_to_SecB", a = "Intermediate epithelium",
       b = "SecB epithelium"),
  list(label = "SecA_to_SecB", a = "SecA epithelium",
       b = "SecB epithelium")
)

cliffs_delta <- function(a, b) {
  a <- a[is.finite(a)]; b <- b[is.finite(b)]
  if (length(a) < 5 || length(b) < 5) return(NA_real_)
  na <- length(a); nb <- length(b)
  ranks_combined <- rank(c(a, b))
  W <- sum(ranks_combined[seq_len(na)]) - na * (na + 1) / 2
  -1 * ((2 * W) / (na * nb) - 1)
}

# ---------------------------------------------------------------------------
# Per-patient eligibility
# ---------------------------------------------------------------------------
tma_n <- tma[cell_label %in% c("SecA epithelium",
                                  "Intermediate epithelium",
                                  "SecB epithelium"),
               .(n = .N), by = .(group_id, cell_label)]
tma_n_wide <- dcast(tma_n, group_id ~ cell_label, value.var = "n")
setnames(tma_n_wide,
          c("SecA epithelium","Intermediate epithelium","SecB epithelium"),
          c("n_SecA","n_Int","n_SecB"), skip_absent = TRUE)
tma_n_wide[, eligible := !is.na(n_SecA) & !is.na(n_Int) & !is.na(n_SecB) &
            n_SecA >= 30 & n_Int >= 30 & n_SecB >= 30]

eligible_patients <- tma_n_wide[eligible == TRUE, group_id]
message(sprintf("Eligible TMA patients: %d / %d",
                length(eligible_patients), nrow(tma_n_wide)))

# ---------------------------------------------------------------------------
# Per-patient median per (cell_label × feature)
# ---------------------------------------------------------------------------
tma_med <- tma[cell_label %in% c("SecA epithelium",
                                    "Intermediate epithelium",
                                    "SecB epithelium") &
                  group_id %in% eligible_patients,
                 lapply(.SD, median, na.rm = TRUE),
                 by = .(group_id, cell_label),
                 .SDcols = FEATURES]

# ---------------------------------------------------------------------------
# Pairwise tests
# ---------------------------------------------------------------------------
out_rows <- list()
for (feat in FEATURES) {
  for (pp in PAIRWISE) {
    per <- dcast(tma_med[cell_label %in% c(pp$a, pp$b)],
                  group_id ~ cell_label, value.var = feat)
    if (!all(c(pp$a, pp$b) %in% colnames(per))) next
    per <- per[!is.na(get(pp$a)) & !is.na(get(pp$b))]
    if (nrow(per) < 5) next
    delta <- per[[pp$b]] - per[[pp$a]]
    pred <- PREDICT[[feat]]

    if (pred == "up") {
      n_concordant <- sum(delta > 0)
      alt <- "less"
    } else {
      n_concordant <- sum(delta < 0)
      alt <- "greater"
    }
    n_concordant_either <- max(sum(delta > 0), sum(delta < 0))

    w  <- wilcox.test(per[[pp$a]], per[[pp$b]], paired = TRUE,
                       alternative = alt, exact = FALSE)
    w2 <- wilcox.test(per[[pp$a]], per[[pp$b]], paired = TRUE,
                       exact = FALSE)

    a_vals <- tma[group_id %in% eligible_patients & cell_label == pp$a,
                    get(feat)]
    b_vals <- tma[group_id %in% eligible_patients & cell_label == pp$b,
                    get(feat)]
    n_sub <- 30000L
    if (length(a_vals) > n_sub) a_vals <- sample(a_vals, n_sub)
    if (length(b_vals) > n_sub) b_vals <- sample(b_vals, n_sub)
    cd <- tryCatch(cliffs_delta(a_vals, b_vals),
                    error = function(e) NA_real_)

    out_rows[[length(out_rows) + 1L]] <- data.table(
      feature       = feat,
      comparison    = pp$label,
      predicted_dir = pred,
      n_patients    = nrow(per),
      n_concordant_predicted = n_concordant,
      n_concordant_either    = n_concordant_either,
      pct_concordant_either  = round(100 * n_concordant_either / nrow(per), 1),
      observed_direction = ifelse(median(delta, na.rm = TRUE) > 0,
                                    "up", "down"),
      median_delta_per_patient = median(delta, na.rm = TRUE),
      paired_wilcox_p_one_sided = w$p.value,
      paired_wilcox_p_two_sided = w2$p.value,
      cliffs_delta_pooled = cd
    )
  }
}
result <- rbindlist(out_rows, fill = TRUE)

# Bonferroni for 27 tests
result[, paired_p_bonf_one_sided := pmin(paired_wilcox_p_one_sided * .N, 1)]
result[, paired_p_bonf_two_sided := pmin(paired_wilcox_p_two_sided * .N, 1)]
result[, validated_per_patient := pct_concordant_either >= 70]
result[, validated_paired_p_two_sided := paired_p_bonf_two_sided < 0.05]

setorder(result, feature, comparison)
fwrite(result, file.path(OUT_DIR, "tma_pairwise_summary.csv"))

cat("\n=== TMA PAIRWISE SUMMARY ===\n")
print(result[, .(feature, comparison, predicted_dir,
                   observed_direction, n_patients,
                   n_concordant_either,
                   pct_concordant_either,
                   median_delta = round(median_delta_per_patient, 3),
                   p_two = signif(paired_wilcox_p_two_sided, 3),
                   p_bonf_two = signif(paired_p_bonf_two_sided, 3),
                   cliffs_delta = round(cliffs_delta_pooled, 3))])

cat("\n=== STRONG TMA SIGNALS (>=70% patients same direction AND Bonferroni 2-sided p < 0.05) ===\n")
strong <- result[pct_concordant_either >= 70 &
                    paired_p_bonf_two_sided < 0.05]
print(strong[, .(feature, comparison, predicted_dir, observed_direction,
                   pct_concordant_either, p_bonf_two = signif(paired_p_bonf_two_sided, 3),
                   median_delta = round(median_delta_per_patient, 3))])

cat(sprintf("\n%d / 27 (feature × comparison) cells show strong TMA signal\n",
             nrow(strong)))

message("\nStep 2 complete.")

# ============================================================================
# 07_bcell_niche_analysis.R
# ----------------------------------------------------------------------------
# PURPOSE: Context-dependent B cell characterization: cells in the immune-rich niche vs cells infiltrating SecB-enriched epithelium (niche-conditioned DEG + correlation).
#
# INPUTS:
#   - SFEs (load_sfe) with cell_label + neighborhood
#
# OUTPUTS:
#   - output/13_macrophage_niche/ bcell niche tables + figures
#
...
out_13c <- file.path(out_dir, "13c_bcell_niche")
if (!dir.exists(out_13c)) dir.create(out_13c, recursive = TRUE)

fig_13c <- file.path(fig_dir, "13c_bcell_niche")
if (!dir.exists(fig_13c)) dir.create(fig_13c, recursive = TRUE)

message("\n=== Script 13c: B Cell Niche Analysis ===")
...
# Naive B cells
naive_genes <- c("IGHD", "TCL1A", "IQCG", "SELL", "IL7R")

# Activated / Memory B cells
activated_genes <- c("TNFRSF13B", "CD27", "CD40", "CD69", "CD80", "CD86")

# Antigen presentation
antigen_pres_genes <- c("CIITA", "TAP1", "TAP2", "TAPBP", "PSME1", "PSME2",
                        "PDIA3", "CALR", "CANX")

# Proliferating B cells
proliferating_genes <- c("MKI67", "STMN1", "TOP2A", "CCNB1", "CDK1",
                          "CDC20", "PCNA")

# Plasma cell differentiation markers (transitional)
plasma_diff_genes <- c("XBP1", "MZB1", "JCHAIN", "DERL3", "SSR4", "FKBP11",
                       "SEC11C")

functional_sets <- list(
  naive          = naive_genes,
  activated      = activated_genes,
  antigen_pres   = antigen_pres_genes,
  proliferating  = proliferating_genes,
  plasma_diff    = plasma_diff_genes
)

# ============================================================================
# STEP 1: Collect B cell data across all SFEs
# ============================================================================

message("\n--- Step 1: Collecting B cell data from all SFEs ---")
...
for (sname in sfe_names) {
  ...
  keep <- !is.na(cd$niche_name) &
          cd$cell_label == "B cell" &
          cd$niche_name %in% c(niche_a, niche_b)

  if (sum(keep) < 10) {
    message("    Skipping: only ", sum(keep), " B cells in target niches")
  ...
  lc <- as.matrix(logcounts(sfe_bc))

  # Build metadata
  meta <- data.frame(
    cell_id    = colnames(sfe_bc),
    sample     = sname,
    niche      = cd_bc$niche_name,
    stringsAsFactors = FALSE
  )

  # Grab pathway scores if present
  pw_cols <- grep("^pathway_", colnames(cd_bc), value = TRUE)
  if (length(pw_cols) > 0) {
    meta <- cbind(meta, cd_bc[, pw_cols, drop = FALSE])
  }

  # Pseudobulk grouping
  if ("core_id" %in% colnames(cd_bc) && !all(is.na(cd_bc$core_id))) {
    meta$group_id <- paste0(sname, "_core", cd_bc$core_id, "_", cd_bc$niche_name)
  } else {
    meta$group_id <- paste0(sname, "_", cd_bc$niche_name)
  }

  bc_meta_list[[sname]] <- meta
  bc_expr_list[[sname]] <- lc

  n_a <- sum(cd_bc$niche_name == niche_a)
  n_b <- sum(cd_bc$niche_name == niche_b)
  message(sprintf("    %s: %d B cells (%d in nb_2, %d in nb_6)",
                  sname, ncol(sfe_bc), n_a, n_b))

  # --- Spatial gradient: B cell coords + distance to epithelial niches ---
  coords <- spatialCoords(sfe)
  bc_idx <- which(keep)
  bc_xy  <- coords[bc_idx, , drop = FALSE]
  ...
  for (enb in epi_nbs) {
    ...
    chunk_size <- 5000
    nn_dists <- numeric(nrow(bc_xy))
    for (ci in seq(1, nrow(bc_xy), by = chunk_size)) {
      ci_end <- min(ci + chunk_size - 1, nrow(bc_xy))
      bc_chunk <- bc_xy[ci:ci_end, , drop = FALSE]
      dx <- outer(bc_chunk[, 1], epi_xy[, 1], "-")
      dy <- outer(bc_chunk[, 2], epi_xy[, 2], "-")
  ...
  rm(sfe, sfe_bc, lc, cd, cd_bc, coords, bc_xy); gc(verbose = FALSE)
}

# Combine across samples
bc_meta <- do.call(rbind, bc_meta_list)
bc_expr <- do.call(cbind, bc_expr_list)

message(sprintf("\nTotal B cells collected: %s",
                format(nrow(bc_meta), big.mark = ",")))
message(sprintf("  %s (nb_2): %s",
                niche_a, format(sum(bc_meta$niche == niche_a), big.mark = ",")))
message(sprintf("  %s (nb_6): %s",
                niche_b, format(sum(bc_meta$niche == niche_b), big.mark = ",")))

# ============================================================================
# STEP 2: Pseudobulk DEG analysis (B cells only, nb_2 vs nb_6)
# ============================================================================

message("\n--- Step 2: Pseudobulk DEG analysis ---")

groups <- unique(bc_meta$group_id)
genes  <- rownames(bc_expr)

group_sizes <- table(bc_meta$group_id)
...
for (grp in valid_groups) {
  idx <- which(bc_meta$group_id == grp)
  pb_counts[, grp] <- rowSums(bc_expr[, idx, drop = FALSE])
}

pb_meta <- data.frame(
  group_id = valid_groups,
  niche    = sapply(valid_groups, function(g) {
    bc_meta$niche[bc_meta$group_id == g][1]
  }),
...
write.csv(deg_results,
          file.path(out_13c, "deg_bcell_nb2_vs_nb6.csv"),
          row.names = FALSE)
...
# ============================================================================
# STEP 3: B Cell Functional State Scoring
# ============================================================================

message("\n--- Step 3: Functional state scoring ---")

panel_genes <- rownames(bc_expr)
functional_scores <- data.frame(niche = bc_meta$niche)

for (set_name in names(functional_sets)) {
  gs <- intersect(functional_sets[[set_name]], panel_genes)
  if (length(gs) >= 2) {
    functional_scores[[set_name]] <- colMeans(bc_expr[gs, , drop = FALSE])
...
# Naive / activated ratio
if (!is.na(functional_scores$naive[1]) && !is.na(functional_scores$activated[1])) {
  functional_scores$naive_activated_ratio <-
    (functional_scores$naive + 0.01) / (functional_scores$activated + 0.01)
}
...
write.csv(func_stats_df,
          file.path(out_13c, "functional_scores_nb2_vs_nb6.csv"),
          row.names = FALSE)
...
# ============================================================================
# STEP 4: Pathway Activity Comparison (B cells only)
# ============================================================================

message("\n--- Step 4: Pathway activity comparison ---")

pw_cols <- grep("^pathway_", colnames(bc_meta), value = TRUE)
...
if (length(pw_cols) > 0) {
  ...
  for (pw in pw_cols) {
    vals_a <- bc_meta[[pw]][bc_meta$niche == niche_a]
    vals_b <- bc_meta[[pw]][bc_meta$niche == niche_b]
  ...
  write.csv(pw_stats_df,
            file.path(out_13c, "pathway_comparison_bcell_nb2_vs_nb6.csv"),
            row.names = FALSE)
...
# B cell-relevant genes for correlation analysis
bc_relevant_genes <- unique(c(
  # Pan B cell markers
  "MS4A1", "CD79A", "CD19", "CD22",
  # Naive
  "IGHD", "TCL1A", "IQCG", "SELL", "IL7R",
  # Activated / memory
  "TNFRSF13B", "CD27", "CD40", "CD69", "CD80", "CD86",
  # Antigen presentation
  "CIITA", "TAP1", "TAP2", "TAPBP", "PSME1", "PSME2", "PDIA3", "CALR", "CANX",
  # Proliferation
  "MKI67", "STMN1", "TOP2A", "PCNA",
  # Plasma differentiation
  "XBP1", "MZB1", "JCHAIN", "DERL3", "SSR4", "FKBP11",
  # Signalling
  "STAT1", "STAT3", "NFKB1", "NFKB2", "IRF1", "IRF8",
  # Chemokines / homing
  "CXCR4", "CCR7", "CXCL9", "CXCL10",
  # Immune regulation
  "IL10", "TGFB1", "CD274", "CTLA4"
))

cor_genes <- intersect(bc_relevant_genes, panel_genes)
message(sprintf("  Correlation analysis: %d B cell-relevant genes on panel", length(cor_genes)))

idx_a <- which(bc_meta$niche == niche_a)
idx_b <- which(bc_meta$niche == niche_b)
...
if (length(cor_genes) >= 5 && length(idx_a) >= 50 && length(idx_b) >= 50) {
  ...
  cor_nb2 <- cor(t(bc_expr[cor_genes, idx_a]), method = "spearman")
  cor_nb6 <- cor(t(bc_expr[cor_genes, idx_b]), method = "spearman")

  cor_diff <- cor_nb2 - cor_nb6

  write.csv(as.data.frame(cor_nb2),
            file.path(out_13c, "correlation_bcell_nb2.csv"))
  write.csv(as.data.frame(cor_nb6),
            file.path(out_13c, "correlation_bcell_nb6.csv"))
  write.csv(as.data.frame(cor_diff),
            file.path(out_13c, "correlation_difference_nb2_minus_nb6.csv"))
  ...
  write.csv(diff_df,
            file.path(out_13c, "differential_correlations_nb2_vs_nb6.csv"),
            row.names = FALSE)
...
func_for_merge <- functional_scores[, setdiff(colnames(functional_scores), "niche"),
                                     drop = FALSE]
func_for_merge$cell_id <- bc_meta$cell_id
...
score_cols <- c("naive", "activated", "antigen_pres", "proliferating",
                "plasma_diff", "naive_activated_ratio")
...
for (sc in score_cols) {
  ...
  agg <- dt[!is.na(score),
            .(mean_score = mean(score),
              se         = sd(score) / sqrt(.N),
              n_cells    = .N),
            by = .(dist_bin, dist_mid, bc_niche, epi_nb)]
  setnames(agg, "bc_niche", "niche")
  agg$score_type <- sc
...
write.csv(grad_agg,
          file.path(out_13c, "spatial_gradient_functional_by_epi_distance.csv"),
          row.names = FALSE)
...
write.csv(grad_cor_df,
          file.path(out_13c, "spatial_gradient_correlations.csv"),
          row.names = FALSE)
...
write.csv(gradient_df[, c("cell_id", "sample", "niche", "epi_nb", "dist_to_epi",
                           score_cols)],
          file.path(out_13c, "spatial_gradient_raw.csv"),
          row.names = FALSE)
...
p_volcano <- ggplot(deg_plot, aes(x = log2FC, y = neg_log10_p, color = category)) +
  ...
  labs(title = "B Cell DEGs: Immune-rich vs SecB-enriched epithelium",
...
ggsave(file.path(fig_13c, "volcano_bcell_nb2_vs_nb6.pdf"),
...
p_func <- ggplot(func_long, aes(x = score_type, y = score, fill = niche)) +
  ...
  scale_fill_manual(values = c("Immune niche" = "#8FBC8F",
                    ...
                    name = "Niche") +
  labs(title = "B cell functional state scores by niche",
       x = NULL, y = "Mean logcounts score") +
  theme_lab(base_size = 8) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1),
        legend.position = "top")

ggsave(file.path(fig_13c, "functional_violin_nb2_vs_nb6.pdf"),
...
if (exists("pw_stats_df")) {
  ...
  p_pathway <- ggplot(pw_stats_df,
    ...
    labs(title = "Pathway activity: B cells in nb_2 vs nb_6",
         subtitle = "Positive = higher in Immune-rich niche",
         x = "Cohen's d (effect size)", y = NULL) +
    theme_lab(base_size = 8)

  ggsave(file.path(fig_13c, "pathway_dotplot_bcell_nb2_vs_nb6.pdf"),
...
if (exists("cor_diff") && length(cor_genes) >= 5) {
  ...
  pdf(file.path(fig_13c, "correlation_heatmap_diff_nb2_minus_nb6.pdf"),
      width = 10, height = 9)
  Heatmap(cor_diff,
          name = "rho diff\n(nb7 - nb1)",
          ...
          column_title = "B cell differential correlation: nb_2 minus nb_6",
          show_row_dend = TRUE,
...
for (sc in score_cols) {
  ...
  p_grad <- ggplot(sub_agg,
    ...
    labs(title = sprintf("Spatial gradient: %s score vs distance to epithelial niche", sc_label),
         subtitle = "B cells in nb_2 and nb_6, binned by distance to nearest epithelial cell",
  ...
  ggsave(file.path(fig_13c, paste0("spatial_gradient_", sc, "_vs_epi_distance.pdf")),
...
if (nrow(grad_cor_df) > 0) {
  ...
  p_grad_heat <- ggplot(grad_cor_df,
    ...
    scale_fill_gradient2(low = "#87CEFA", mid = "white", high = "#8FBC8F",
                         ...
                         limits = c(-0.3, 0.3)) +
    labs(title = "B cell functional score-distance correlations by niche",
         subtitle = "Spearman rho (score ~ distance); * padj<0.05, ** <0.01, *** <0.001",
         x = NULL, y = NULL) +
    theme_lab(base_size = 8) +
    theme(axis.text.x = element_text(angle = 35, hjust = 1))

  ggsave(file.path(fig_13c, "spatial_gradient_correlation_heatmap.pdf"),
...
summary_df <- data.frame(
  metric = c(
    "Total B cells analyzed",
    "B cells in nb_2 (Immune niche)",
    "B cells in nb_6 (SecB-enriched epithelium)",
  ...
  value = c(
    nrow(bc_meta),
    sum(bc_meta$niche == niche_a),
    sum(bc_meta$niche == niche_b),
    length(valid_groups),
    n_sig,
    n_up,
    n_dn,
...
write.csv(summary_df,
          file.path(out_13c, "analysis_summary.csv"),
          row.names = FALSE)
...
message("\n=== Script 13c Complete ===")
message("Outputs saved to: ", out_13c)
message("Figures saved to: ", fig_13c)
message("\nFiles generated:")
message("  - deg_bcell_nb2_vs_nb6.csv")
message("  - functional_scores_nb2_vs_nb6.csv")
message("  - pathway_comparison_bcell_nb2_vs_nb6.csv")
message("  - correlation_bcell_nb2.csv / nb1.csv / difference.csv")
...
message("  - volcano_bcell_nb2_vs_nb6.pdf")
message("  - functional_violin_nb2_vs_nb6.pdf")
message("  - pathway_dotplot_bcell_nb2_vs_nb6.pdf")

# ============================================================================
# 06_tcell_niche_analysis.R
# ----------------------------------------------------------------------------
# PURPOSE: Context-dependent T cell characterization: cells in the immune-rich niche vs cells infiltrating SecB-enriched epithelium (niche-conditioned DEG + correlation).
#
# INPUTS:
#   - SFEs (load_sfe) with cell_label + neighborhood
#
# OUTPUTS:
#   - output/13_macrophage_niche/ tcell niche tables + figures
#
...
out_13b <- file.path(out_dir, "13b_tcell_niche")
if (!dir.exists(out_13b)) dir.create(out_13b, recursive = TRUE)

fig_13b <- file.path(fig_dir, "13b_tcell_niche")
if (!dir.exists(fig_13b)) dir.create(fig_13b, recursive = TRUE)

message("\n=== Script 13b: T Cell Niche Analysis ===")
...
# --- T cell functional state gene sets (curated for Xenium 541-gene panel) ---

# Cytotoxic / effector CD8 T cells
cytotoxic_genes <- c("GZMA", "GZMB", "GZMH", "PRF1", "FGFBP2", "CST7",
                     "NKG7", "GNLY", "FASLG", "KLRD1", "KLRB1")

# Regulatory T cells (Treg)
treg_genes <- c("BATF", "TIGIT", "TNFRSF18", "IL2RA", "CTLA4",
                "IL10", "STAT5A", "TGFB1")

# T cell exhaustion / dysfunction
exhaustion_genes <- c("PDCD1", "HAVCR2", "LAG3", "TIGIT", "CTLA4",
                      "BTLA", "CD274", "TOX")

# Naive / resting T cells
naive_genes <- c("IL7R", "SELL", "TCF7", "LTB", "CCR7", "LEF1")

# T cell activation / costimulation
activation_genes <- c("CD27", "CD28", "ICOS", "CD40LG", "IFNG",
...
# ============================================================================
# STEP 1: Collect T cell data across all SFEs
# ============================================================================

message("\n--- Step 1: Collecting T cell data from all SFEs ---")
...
for (sname in sfe_names) {
  ...
  keep <- !is.na(cd$niche_name) &
          cd$cell_label == "T cell" &
          cd$niche_name %in% c(niche_a, niche_b)

  if (sum(keep) < 10) {
    message("    Skipping: only ", sum(keep), " T cells in target niches")
  ...
  lc <- as.matrix(logcounts(sfe_tc))

  # Build metadata
  meta <- data.frame(
    cell_id    = colnames(sfe_tc),
    sample     = sname,
    niche      = cd_tc$niche_name,
    stringsAsFactors = FALSE
  )

  # Grab pathway scores if present
  pw_cols <- grep("^pathway_", colnames(cd_tc), value = TRUE)
  if (length(pw_cols) > 0) {
    meta <- cbind(meta, cd_tc[, pw_cols, drop = FALSE])
  }

  # Pseudobulk grouping
  if ("core_id" %in% colnames(cd_tc) && !all(is.na(cd_tc$core_id))) {
    meta$group_id <- paste0(sname, "_core", cd_tc$core_id, "_", cd_tc$niche_name)
  } else {
    meta$group_id <- paste0(sname, "_", cd_tc$niche_name)
  }

  tc_meta_list[[sname]] <- meta
  tc_expr_list[[sname]] <- lc

  n_a <- sum(cd_tc$niche_name == niche_a)
  n_b <- sum(cd_tc$niche_name == niche_b)
  message(sprintf("    %s: %d T cells (%d in nb_2, %d in nb_6)",
                  sname, ncol(sfe_tc), n_a, n_b))

  # --- Spatial gradient: T cell coords + distance to epithelial niches ---
  coords <- spatialCoords(sfe)
  tc_idx <- which(keep)
  tc_xy  <- coords[tc_idx, , drop = FALSE]
  ...
  for (enb in epi_nbs) {
    ...
    chunk_size <- 5000
    nn_dists <- numeric(nrow(tc_xy))
    for (ci in seq(1, nrow(tc_xy), by = chunk_size)) {
      ci_end <- min(ci + chunk_size - 1, nrow(tc_xy))
      tc_chunk <- tc_xy[ci:ci_end, , drop = FALSE]
      dx <- outer(tc_chunk[, 1], epi_xy[, 1], "-")
      dy <- outer(tc_chunk[, 2], epi_xy[, 2], "-")
  ...
  rm(sfe, sfe_tc, lc, cd, cd_tc, coords, tc_xy); gc(verbose = FALSE)
}

# Combine across samples
tc_meta <- do.call(rbind, tc_meta_list)
tc_expr <- do.call(cbind, tc_expr_list)

message(sprintf("\nTotal T cells collected: %s",
                format(nrow(tc_meta), big.mark = ",")))
message(sprintf("  %s (nb_2): %s",
                niche_a, format(sum(tc_meta$niche == niche_a), big.mark = ",")))
message(sprintf("  %s (nb_6): %s",
                niche_b, format(sum(tc_meta$niche == niche_b), big.mark = ",")))

# ============================================================================
# STEP 2: Pseudobulk DEG analysis (T cells only, nb_2 vs nb_6)
# ============================================================================

message("\n--- Step 2: Pseudobulk DEG analysis ---")

groups <- unique(tc_meta$group_id)
genes  <- rownames(tc_expr)

group_sizes <- table(tc_meta$group_id)
...
for (grp in valid_groups) {
  idx <- which(tc_meta$group_id == grp)
  pb_counts[, grp] <- rowSums(tc_expr[, idx, drop = FALSE])
}

pb_meta <- data.frame(
  group_id = valid_groups,
  niche    = sapply(valid_groups, function(g) {
    tc_meta$niche[tc_meta$group_id == g][1]
  }),
...
write.csv(deg_results,
          file.path(out_13b, "deg_tcell_nb2_vs_nb6.csv"),
          row.names = FALSE)
...
# ============================================================================
# STEP 3: T Cell Functional State Scoring
# ============================================================================

message("\n--- Step 3: Functional state scoring ---")

panel_genes <- rownames(tc_expr)
functional_scores <- data.frame(niche = tc_meta$niche)

for (set_name in names(functional_sets)) {
  gs <- intersect(functional_sets[[set_name]], panel_genes)
  if (length(gs) >= 2) {
    functional_scores[[set_name]] <- colMeans(tc_expr[gs, , drop = FALSE])
...
# Cytotoxic / exhaustion ratio
if (!is.na(functional_scores$cytotoxic[1]) && !is.na(functional_scores$exhaustion[1])) {
  functional_scores$cytotoxic_exhaustion_ratio <-
    (functional_scores$cytotoxic + 0.01) / (functional_scores$exhaustion + 0.01)
}
...
write.csv(func_stats_df,
          file.path(out_13b, "functional_scores_nb2_vs_nb6.csv"),
          row.names = FALSE)
...
# ============================================================================
# STEP 4: Pathway Activity Comparison (T cells only)
# ============================================================================

message("\n--- Step 4: Pathway activity comparison ---")

pw_cols <- grep("^pathway_", colnames(tc_meta), value = TRUE)
...
if (length(pw_cols) > 0) {
  ...
  for (pw in pw_cols) {
    vals_a <- tc_meta[[pw]][tc_meta$niche == niche_a]
    vals_b <- tc_meta[[pw]][tc_meta$niche == niche_b]
  ...
  write.csv(pw_stats_df,
            file.path(out_13b, "pathway_comparison_tcell_nb2_vs_nb6.csv"),
            row.names = FALSE)
...
# T cell-relevant genes for correlation analysis
tc_relevant_genes <- unique(c(
  # Pan T cell markers
  "CD3E", "CD3D", "CD3G", "CD2", "TRAC",
  # CD8 effector / cytotoxic
  "GZMA", "GZMB", "GZMH", "PRF1", "FGFBP2", "CST7", "NKG7", "GNLY",
  # CD4 helper / Treg
  "BATF", "IL2RA", "TNFRSF18", "TIGIT", "CTLA4", "IL10",
  # Exhaustion / checkpoint
  "PDCD1", "HAVCR2", "LAG3", "BTLA", "CD274",
  # Naive / memory
  "IL7R", "SELL", "TCF7", "LTB", "CCR7",
  # Activation
  "CD27", "IFNG", "TNF", "LCK", "ZAP70", "IL2RG",
  # Signalling / transcription
  "STAT1", "STAT3", "STAT4", "JAK1", "JAK3", "IRF1",
  # Chemokines / homing
  "CXCR4", "CXCR6", "CCR5", "CXCL9", "CXCL10", "CXCL11",
  # Costimulation
  "CD86", "ICOS", "FASLG"
))

cor_genes <- intersect(tc_relevant_genes, panel_genes)
message(sprintf("  Correlation analysis: %d T cell-relevant genes on panel", length(cor_genes)))

idx_a <- which(tc_meta$niche == niche_a)
idx_b <- which(tc_meta$niche == niche_b)
...
if (length(cor_genes) >= 5 && length(idx_a) >= 50 && length(idx_b) >= 50) {
  ...
  cor_nb2 <- cor(t(tc_expr[cor_genes, idx_a]), method = "spearman")
  cor_nb6 <- cor(t(tc_expr[cor_genes, idx_b]), method = "spearman")

  cor_diff <- cor_nb2 - cor_nb6

  write.csv(as.data.frame(cor_nb2),
            file.path(out_13b, "correlation_tcell_nb2.csv"))
  write.csv(as.data.frame(cor_nb6),
            file.path(out_13b, "correlation_tcell_nb6.csv"))
  write.csv(as.data.frame(cor_diff),
            file.path(out_13b, "correlation_difference_nb2_minus_nb6.csv"))
  ...
  write.csv(diff_df,
            file.path(out_13b, "differential_correlations_nb2_vs_nb6.csv"),
            row.names = FALSE)
...
func_for_merge <- functional_scores[, setdiff(colnames(functional_scores), "niche"),
                                     drop = FALSE]
func_for_merge$cell_id <- tc_meta$cell_id
...
score_cols <- c("cytotoxic", "treg", "exhaustion", "naive", "activation",
                "cytotoxic_exhaustion_ratio")
...
for (sc in score_cols) {
  ...
  agg <- dt[!is.na(score),
            .(mean_score = mean(score),
              se         = sd(score) / sqrt(.N),
              n_cells    = .N),
            by = .(dist_bin, dist_mid, tc_niche, epi_nb)]
  setnames(agg, "tc_niche", "niche")
  agg$score_type <- sc
...
write.csv(grad_agg,
          file.path(out_13b, "spatial_gradient_functional_by_epi_distance.csv"),
          row.names = FALSE)
...
write.csv(grad_cor_df,
          file.path(out_13b, "spatial_gradient_correlations.csv"),
          row.names = FALSE)
...
write.csv(gradient_df[, c("cell_id", "sample", "niche", "epi_nb", "dist_to_epi",
                           score_cols)],
          file.path(out_13b, "spatial_gradient_raw.csv"),
          row.names = FALSE)
...
p_volcano <- ggplot(deg_plot, aes(x = log2FC, y = neg_log10_p, color = category)) +
  ...
  labs(title = "T Cell DEGs: Immune-rich vs SecB-enriched epithelium",
...
ggsave(file.path(fig_13b, "volcano_tcell_nb2_vs_nb6.pdf"),
...
p_func <- ggplot(func_long, aes(x = score_type, y = score, fill = niche)) +
  ...
  scale_fill_manual(values = c("Immune niche" = "#8FBC8F",
                    ...
                    name = "Niche") +
  labs(title = "T cell functional state scores by niche",
       x = NULL, y = "Mean logcounts score") +
  theme_lab(base_size = 8) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1),
        legend.position = "top")

ggsave(file.path(fig_13b, "functional_violin_nb2_vs_nb6.pdf"),
...
if (exists("pw_stats_df")) {
  ...
  p_pathway <- ggplot(pw_stats_df,
    ...
    labs(title = "Pathway activity: T cells in nb_2 vs nb_6",
         subtitle = "Positive = higher in Immune-rich niche",
         x = "Cohen's d (effect size)", y = NULL) +
    theme_lab(base_size = 8)

  ggsave(file.path(fig_13b, "pathway_dotplot_tcell_nb2_vs_nb6.pdf"),
...
if (exists("cor_diff") && length(cor_genes) >= 5) {
  ...
  pdf(file.path(fig_13b, "correlation_heatmap_diff_nb2_minus_nb6.pdf"),
      width = 10, height = 9)
  Heatmap(cor_diff,
          name = "rho diff\n(nb7 - nb1)",
          ...
          column_title = "T cell differential correlation: nb_2 minus nb_6",
          show_row_dend = TRUE,
...
for (sc in score_cols) {
  ...
  p_grad <- ggplot(sub_agg,
    ...
    labs(title = sprintf("Spatial gradient: %s score vs distance to epithelial niche", sc_label),
         subtitle = "T cells in nb_2 and nb_6, binned by distance to nearest epithelial cell",
  ...
  ggsave(file.path(fig_13b, paste0("spatial_gradient_", sc, "_vs_epi_distance.pdf")),
...
if (nrow(grad_cor_df) > 0) {
  ...
  p_grad_heat <- ggplot(grad_cor_df,
    ...
    scale_fill_gradient2(low = "#87CEFA", mid = "white", high = "#8FBC8F",
                         ...
                         limits = c(-0.3, 0.3)) +
    labs(title = "T cell functional score-distance correlations by niche",
         subtitle = "Spearman rho (score ~ distance); * padj<0.05, ** <0.01, *** <0.001",
         x = NULL, y = NULL) +
    theme_lab(base_size = 8) +
    theme(axis.text.x = element_text(angle = 35, hjust = 1))

  ggsave(file.path(fig_13b, "spatial_gradient_correlation_heatmap.pdf"),
...
summary_df <- data.frame(
  metric = c(
    "Total T cells analyzed",
    "T cells in nb_2 (Immune niche)",
    "T cells in nb_6 (SecB-enriched epithelium)",
  ...
  value = c(
    nrow(tc_meta),
    sum(tc_meta$niche == niche_a),
    sum(tc_meta$niche == niche_b),
    length(valid_groups),
    n_sig,
    n_up,
    n_dn,
...
write.csv(summary_df,
          file.path(out_13b, "analysis_summary.csv"),
          row.names = FALSE)
...
message("\n=== Script 13b Complete ===")
message("Outputs saved to: ", out_13b)
message("Figures saved to: ", fig_13b)
message("\nFiles generated:")
message("  - deg_tcell_nb2_vs_nb6.csv")
message("  - functional_scores_nb2_vs_nb6.csv")
message("  - pathway_comparison_tcell_nb2_vs_nb6.csv")
message("  - correlation_tcell_nb2.csv / nb1.csv / difference.csv")
...
message("  - volcano_tcell_nb2_vs_nb6.pdf")
message("  - functional_violin_nb2_vs_nb6.pdf")
message("  - pathway_dotplot_tcell_nb2_vs_nb6.pdf")

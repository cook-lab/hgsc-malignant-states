#!/usr/bin/env python3
"""
Atlas 02 — Step 12b (proportions): cell-type composition + figures

PURPOSE
    Compute level-1 and level-2 cell-type proportions across metadata strata, with
    the epithelial SecA/SecB polarization breakdown, and render composition figures
    + tables. Supersedes step 12.

INPUTS
    obj("atlas_celltype_l2")  = hgsc_atlas_celltype_level2.h5ad

OUTPUTS
    output_root/02_annotation/12b_celltype_proportions/{figs,tables}/*, report PDF

MANUSCRIPT PANEL(S)
    Fig 1B-E composition substrate; epithelial proportions for Fig 2D/SF9.
...
PDF_PATH    = os.path.join(OUT_DIR, "12b_celltype_proportions_report.pdf")
...
# Cell type palette v1.2 (Cook Lab guide — level1 labels)
CELLTYPE_PALETTE = {
    "Epithelial":    "#E6A141",
    "Mesothelial":   "#A8A298",
    "Fibroblast":    "#DDD5CA",
...
# Study palette (Kelly 22, first 13)
KELLY_22 = [
...
# Level2 color sequence — from 9e, maximally-contrasting, level1-divergent
...
EPITHELIAL_PALETTE = {
    "Adaptive secretory epithelial cell":        "#B8741A",  # SecB (dark amber)
    "Ciliated epithelial cell":                  "#E05A2C",  # Cil  (orange-red)
    "Cycling secretory epithelial cell":         "#F6D28B",  # SecA (light gold)
    "Secretory epithelial cell":                 "#E6A141",  # Intermediate (golden)
    "Stress-response secretory epithelial cell": "#D9C5A2",  # Epi  (pale sand)
    "Transitioning epithelial cell":             "#7D4E4E",  # dark brown
}
...
# Boolean-like columns where "False" means not assessed
...
def clean_metadata(obs, col):
    ...
    Returns a cleaned copy of the column as a string Series.
    """
...
def compute_proportions(obs, group_col, category_col, category_order=None):
    """
    Compute cell type proportions per group.

    Returns (props_df, counts_df):
        props_df  — rows=groups, cols=categories, values in [0,1]
        counts_df — raw counts
    """
    counts = pd.crosstab(obs[group_col], obs[category_col])
    props = pd.crosstab(obs[group_col], obs[category_col], normalize="index")

    # Remove rows with zero total cells
    nonzero = counts.sum(axis=1) > 0
    counts = counts.loc[nonzero]
    props = props.loc[nonzero]

    # Order columns: use provided order (for level1), else by descending frequency
...
def get_metadata_palette(meta_key, categories):
    ...
    # Ensure NA/Unknown is always grey
...
def get_level2_palette(compartment_level1, categories):
    ...
    For epithelial compartment, uses EPITHELIAL_PALETTE.
    For all others, assigns sorted subtypes to LEVEL2_COLOR_SEQUENCE
    (matching 9e's assign_subtype_colors).
    """
...
def plot_stacked_bar(proportions, palette, title, xlabel="Proportion",
                     ylabel=None, annotations=None):
    """
    Horizontal stacked bar chart.

    proportions: DataFrame, rows=groups, cols=categories
    palette: dict mapping category → color
    annotations: optional dict mapping group → subtitle string (e.g. "n=1,234, s=45")
    Returns matplotlib Figure.
    """
    n = len(proportions)
    has_annot = annotations is not None and len(annotations) > 0
    row_height = 0.50 if has_annot else 0.35
    fig_h = max(4, n * row_height)
    fig_w = max(7, n * 0.15 + 5)
    ...
    for col in proportions.columns:
        color = palette.get(col, "#CCCCCC")
        vals = proportions[col].values
        ax.barh(y_pos, vals, left=left, color=color, edgecolor="white",
                linewidth=0.3, height=0.8, label=col)
        left += vals

    ax.set_yticks(y_pos)
    tick_fs = max(5, min(7, 200 // max(n, 1)))
    if has_annot:
        labels = []
        for l in proportions.index:
            lbl = abbreviate_label(l)
            if l in annotations:
                lbl = f"{lbl}\n{annotations[l]}"
            labels.append(lbl)
        ax.set_yticklabels(labels, fontsize=tick_fs, linespacing=1.4)
    else:
        labels = [abbreviate_label(l) for l in proportions.index]
    ...
    ax.legend(handles, labels_leg, bbox_to_anchor=(1.02, 1), loc="upper left",
              fontsize=6, frameon=False, ncol=1 if len(proportions.columns) <= 15 else 2)
...
# ============================================================================
# PLOTTING — PATIENT HEATMAP
# ============================================================================

def plot_patient_heatmap(proportions, title, celltype_colors=None):
    """
    Clustered heatmap for patient_id × cell type proportions.
    Returns a seaborn ClusterGrid figure.
    """
    # Build column color bar
    col_colors = None
    if celltype_colors:
        col_colors = pd.Series(
            {c: celltype_colors.get(c, "#CCCCCC") for c in proportions.columns},
            name="Cell type"
        )

    n_patients = len(proportions)
    n_cols = len(proportions.columns)
    fig_h = max(8, n_patients * 0.08 + 2)

    # Disable clustering on axes with ≤1 item (can't compute distances)
...
def plot_patient_top30_bar(proportions, palette, title):
    """Stacked bar for top-30 patients by cell count (supplemental)."""
    if len(proportions) <= 30:
        return plot_stacked_bar(proportions, palette, title)
    top30 = proportions.iloc[:30]
    return plot_stacked_bar(top30, palette,
                            title + " (Top 30 by cell count)")


# ============================================================================
# PDF HELPER — save figure to both PdfPages and individual PDF
# ============================================================================
...
def render_title_page(pdf, obs):
    ...
    ax.text(0.5, 0.90, "Cell Type Proportion Analysis",
            transform=ax.transAxes, fontsize=20, fontweight="bold",
            ha="center", va="top")
    ax.text(0.5, 0.85, "HGSC Atlas | Cook Lab | Step 12b",
    ...
    summary_df.to_csv(os.path.join(TABLE_DIR, "12b_metadata_summary.csv"),
                      index=False)
    print("  Saved metadata summary table")


# ============================================================================
# PART 1 — LEVEL 1 PROPORTIONS
# ============================================================================

def render_part1(pdf, obs, skip_dataset=False):
    """
    Part 1: celltype_level1 proportions across each metadata variable.
    """
    print("\n" + "=" * 70)
    print("PART 1: Level 1 cell type proportions")
    print("=" * 70)
    ...
    ax = fig.add_subplot(111)
    ax.axis("off")
    ax.text(0.5, 0.55, "Part 1: celltype_level1 Proportions",
    ...
    for meta_key, meta_info in METADATA_VARS.items():
        ...
        # Skip dataset if identical to study
        ...
        # Compute proportions
        props, counts = compute_proportions(work, "_group", "celltype_level1",
        ...
        # Save counts CSV
        csv_name = f"12b_level1_counts_{meta_key}.csv"
        counts.to_csv(os.path.join(TABLE_DIR, csv_name))

        # Special handling for patient_id
        if meta_key == "patient_id":
            # Sort by total cells for top-30 bar
            cell_totals = counts.sum(axis=1).sort_values(ascending=False)
            props_sorted = props.loc[cell_totals.index]

            # Heatmap
            fig_hm = plot_patient_heatmap(
                props, f"Level 1 Proportions × {display}",
                celltype_colors=CELLTYPE_PALETTE
            )
            hm_name = f"12b_level1_vs_{meta_key}_heatmap.pdf"
            save_figure(fig_hm, pdf, hm_name)
            fig_count += 1

            # Top-30 stacked bar
            fig_bar = plot_patient_top30_bar(
                props_sorted, CELLTYPE_PALETTE,
                f"Level 1 Proportions × {display}"
            )
            bar_name = f"12b_level1_vs_{meta_key}_top30.pdf"
            save_figure(fig_bar, pdf, bar_name)
            fig_count += 1

        else:
            # Standard stacked bar
            fig_bar = plot_stacked_bar(
                props, CELLTYPE_PALETTE,
                f"Level 1 Proportions × {display}",
                ylabel=display
            )
            fig_name = f"12b_level1_vs_{meta_key}.pdf"
            save_figure(fig_bar, pdf, fig_name)
            fig_count += 1

        elapsed = time.time() - t0
        print(f"    {len(props)} groups, {len(props.columns)} types "
              f"({elapsed:.1f}s)")
...
# ============================================================================
# PART 2 — LEVEL 2 PROPORTIONS PER COMPARTMENT
# ============================================================================

def render_part2(pdf, obs, compartment_key=None):
    """
    Part 2: celltype_level2 proportions across metadata, split by compartment.
    Includes n= (cell count) and s= (sample count) in each title.
    """
    print("\n" + "=" * 70)
    print("PART 2: Level 2 cell type proportions per compartment")
    print("=" * 70)
    ...
    ax = fig.add_subplot(111)
    ax.axis("off")
    ax.text(0.5, 0.55, "Part 2: celltype_level2 Proportions by Compartment",
            transform=ax.transAxes, fontsize=18, fontweight="bold",
            ha="center", va="center")
    ...
    # Determine which compartments to process
    if compartment_key:
    ...
    for comp_key, comp_info in comp_iter:
        ...
        for meta_key, meta_info in METADATA_VARS.items():
            ...
            # Compute proportions
            props, counts = compute_proportions(work, "_group",
                                                "celltype_level2")

            if len(props) == 0 or len(props.columns) == 0:
                continue

            # Save counts
            csv_name = f"12b_{comp_key}_level2_counts_{meta_key}.csv"
            counts.to_csv(os.path.join(TABLE_DIR, csv_name))

            # Build palette (9e-consistent)
            pal = get_level2_palette(level1_name, props.columns.tolist())

            title = (f"{level1_name} — Level 2 Proportions × {display}"
                     f"  (n={n_comp:,}, s={n_samples})")

            if meta_key == "patient_id":
                # Heatmap for patient_id — skip if only 1 type (nothing to cluster)
                if len(props.columns) < 2:
                    continue
                fig_hm = plot_patient_heatmap(props, title)
                hm_name = f"12b_{comp_key}_level2_vs_{meta_key}_heatmap.pdf"
                save_figure(fig_hm, pdf, hm_name)
                fig_count += 1
            else:
                # Per-group n= and s= annotations
                group_cells = counts.sum(axis=1)
                pid_col = obs_comp["patient_id"].astype(str)
                group_samples = (
                ...
                annot = {
                    ...
                    for g in props.index
                }

                # Stacked bar
                fig_bar = plot_stacked_bar(props, pal, title, ylabel=display,
                                           annotations=annot)
                fig_name = f"12b_{comp_key}_level2_vs_{meta_key}.pdf"
                save_figure(fig_bar, pdf, fig_name)
                fig_count += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Step 12b — Cell Type Proportion Analysis (Updated)"
    )
    ...
    print("=" * 70)
    print("Step 12b — Cell Type Proportion Analysis (Updated)")
    print("=" * 70)
    t_start = time.time()
    ...
    # Check if dataset == study (skip dataset if identical)
    skip_dataset = False

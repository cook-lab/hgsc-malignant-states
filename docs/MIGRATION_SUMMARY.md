...
- **Integration** (CANONICAL (a) step 4): the **official cluster scripts have been located and migrated** (superseding both the earlier interim reconstruction and a still-earlier `atlas_00..05` notebook vintage). The official chain (`01_preprocess` … `05_finalize`) is migrated into `atlas/01_preprocess_qc/` as: `03_preprocess_hvg.py` (HVG on the QC'd concat) → `04_cellassign.py` (81×16 `cellassign_markers_v3.csv`) → five benchmarked integration methods `05a_integrate_scvi.py` / `05b_integrate_scanvi.py` / `05c_integrate_mrvi.py` / `05d_integrate_sysvi.py` / `05e_integrate_harmony.py` → `06_benchmark.py` (scib-metrics) → `07_finalize.py`. These are the real path-centralised originals (analytical parameters unchanged), not reconstructions. **All earlier reconciliation flags are resolved**: (i) QC/Scrublet placement — the official preprocess/cellassign/finalize all read `atlas_concatenated_filtered.h5ad`, the output of `02_concat_qc_doublets.py` (Scrublet < 0.3), so QC IS that step; (ii) object naming — `07_finalize.py --method scanvi` emits `integrated_scanvi.h5ad` with obsm `X_scanvi` (lowercase), exactly what `08_refilter_umap.py` reads → deposited `hgsc_atlas_scanvi.h5ad`; (iii) markers — single canonical `cellassign_markers_v3.csv` (81×16); (iv) method — **scANVI SELECTED** among the five benchmarked. The earlier-vintage migrated files (`02_aggregate`/`04_scvi`/`05_cellassign`/`06_scanvi`/`07_process`/`04b_harmony_comparison`/old `03_preprocess_hvg`) and the stale 53×11 `shared/cellassign_markers.csv` were removed; the interim `03_integration_scvi_scanvi.py` had already been removed. The integrated atlas remains deposited as a convenience entry object; the GPU steps are optional to re-run. Superseded notebook-vintage originals not migrated: `atlas_04_annotation.py`, `atlas_integration.py`, `atlas_integration2.py`.
